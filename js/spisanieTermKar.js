function insertTermKar(data) {
  dataIns = [[data[0], data[4], '', data[1], true]]
  if (data[3] === 'термінал') {
...
//  let valueReestrTrataNaznechenie = valueReestrTrata.map(x => x[3]);
  ...
  if (data[3] === 'термінал') {
   // if (valueReestrTrataNaznechenie.toString().includes(valueVhodSchet) == false) {
      let res = [Utilities.formatDate(new Date(data[0]), 'Europe/Kiev', 'dd.MM.yyyy'), data[1], 'тов', '', 'АВТОМОБІЛЬНІ ІНТЕЛЕКТУАЛЬНІ ТЕХНОЛОГІЇ ' + 'пп' + data[4]];
      sheetReestrTrata.appendRow(res);
   // }
  }
  if (data[3] === 'карантин') {
  //  if (valueReestrTrataNaznechenie.toString().includes(valueVhodSchet) == false) {
      let res = [Utilities.formatDate(new Date(data[0]), 'Europe/Kiev', 'dd.MM.yyyy'), data[1], 'тов', '', 'КАРАНТИН ' + '' + data[4]];
      sheetReestrTrata.appendRow(res);
  //  }
  }
}